...
// current view would live only in React state, so an F5 or a shared link always
// drops you back on the first page. This maps the URL path to a view so a reload
// and a deep link both land where you expect, and the address bar shows the real
// page (/permissions, /config/logging, ...).
//
...
// Views whose second path segment names a thing inside them, e.g.
// /config/logging opens that feature's detail panel directly.
const DETAIL_VIEWS = new Set(['config']);
...
export function parseRoute(pathname) {
  ...
  // A feature id, so a junk path shows the tile list rather than an empty
  // detail panel.
  const param = raw && /^[a-zA-Z][a-zA-Z0-9]*$/.test(raw) ? raw : null;
  return { view, param };
}
